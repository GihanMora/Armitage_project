...
def adding_ig(link_list):
    ...
    for id_a in link_list:
        ...
        mycol.update_one({"link": id_a}, {'$set': {'ignore_flag': '0'}})
...
def get_entries_project(project_id):
    ...
    # selected_project = proj_data_entry
    ...
    if ('associated_queries' in proj_attribute_keys):
        ...
        for each_query in associated_queries:
            ...
            if ('associated_entries' in query_attribute_keys):
                ...
                for id_a in obs_ids:
                    entry = mycol.find({"_id": id_a,"query_id":ObjectId(each_query)})
                    data = [d for d in entry]
                    if(len(data)==0):continue
                    if (data[0]['link'] in link_list):
                        print(data[0]['_id'], data[0]['link'])
...
def set_ig_proj(proj_id):
    projs_list = get_entries_project(proj_id)
    mycol = refer_collection()
    for id_a in projs_list:
...
# set_ig_proj(ObjectId('60cf5241df8355fbc5544a6c'))
# adding_ig(link_list[50:])